class SettingsController < ApplicationController
  ...
  def apply_to_checked_items
    if params[:action_to_perform] == 'Delete'    
        Setting.destroy(params[:delchk].keys) if params[:delchk] && params[:delchk].keys.size > 0
    end 
  ...
  def replace_column_value
    ...
    Setting.update_all("#{params[:field]} = '#{params[:new_value].strip}'", where_clause)
    redirect_to :back
  end
  ...
  def delete_matching_rows
    ...
    Setting.delete_all(where_clause)
    redirect_to :back
  end
  ...
  def quick_add_items
    ...
    if str_names.length > 0 then
      ...
      for str_name in names do
        str_name.strip!
        Setting.find_or_create_by_name(str_name)
      end
    end
  ...
  def create
    sanitize_params(:setting)
    @new_obj = @setting = Setting.new(params[:setting])
    ...
    if ! ok_to_add_or_update?
      ...
      respond_to do |format|
        if @setting.save
          ...
          format.html {
            flash[:notice] = "#{@setting.iname} was successfully created."
            redirected_to_aggregator = update_aggregator(@setting,params)
            if ( ! redirected_to_aggregator ) && session[:post_create_redirect]
                url = session[:post_create_redirect] + "?setting_id=" + @setting.id.to_s + "&setting_ids[]=" + @setting.id.to_s 
                redirect_to url
                session[:post_create_redirect] = nil
            else 
                redirect_to :id => @setting.id, :action => (params[:commit].to_s.include?("Another") ? 'new' : 'edit') if ! redirected_to_aggregator 
            end
          }
        else
          format.xml  { render :xml => @setting.errors, :status => :unprocessable_entity }
          format.html {
            flash[:notice] = 'Setting could not be created. See errors.'
            render :action => 'new'
          }
        end
      end
  ...
  def update
    @updated_obj = @setting = Setting.find(params[:id])
    sanitize_params(:setting)
    ...
    if ! ok_to_add_or_update?
      ...
      @ft_subject = "Setting"
      @ft_activity = "Edit"
      redirect_to :action => 'edit', :id => @setting and return
    end 
  ...
  def ok_to_add_or_update?
    if params[:setting][:name].to_s.empty?
  ...
  def upload_csv
    ...
    #fn = "#{RAILS_ROOT}/uploads/setting_data.csv"
    #File.open(fn, "wb") { |f| f.write(params[:csv_file].read) }
    #Setting.load_csv_file(fn) if Setting.respond_to? :load_csv_file

    Setting.load_csv_str(params[:csv_file].read) if Setting.respond_to? :load_csv_str
  ...
  def download_csv
    if params[:field_list].nil? || @user_is_admin
      attr_names = Setting.value_attrs
    ...
    send_data csv_string, :filename => 'setting_data.csv', :type => 'text/csv'
  end 
  ...
  def run_query
    ...
    @query = Setting.query_specs[params[:query]]
    ...
    @count = Setting.count_by_sql("SELECT COUNT(id) FROM (#{@base_sql}) AS row_data")
    
    @sql = @base_sql + (@sort_fields ? " ORDER BY #{@sort_fields}" : "") + (@page_clause ? " #{@page_clause}" : "") 
    @rows = Setting.find_by_sql(@sql)

    @cols = @query[:cols] || (@rows.size > 0 && @rows[0].attributes.keys)

    @link_params = { :controller => 'settings', :action => 'show' }
  ...
  def prepare_list
    ...
    @ft_subject  = ( @ft_subject || "Setting".pluralize )
    ...
    adjust_sorting_and_filtering_params(Setting,params)
