...
module ApplicationHelper
    ...
    def js_button_to(name,params={},opts={})
        opts = opts.merge({ :class => "js_button #{opts[:class].to_s}".strip })
        url = url_for params
        text = '<input type="submit" value="' + name + '" onClick="location=' + "'" + url + "'" + '; return false;" ' + to_html_attrs(opts) + ' >'
        return text
    end
    ...
    def js_button_with_confirm_to(name,params={},opts={})
        opts = opts.merge({ :class => "js_button #{opts[:class].to_s}".strip })
        url = url_for params
        text = '<input type="submit" value="' + name + '" onClick="if (confirmSubmit()) {location=' + "'" + url + "'" + '; return false;}" ' + to_html_attrs(opts) + ' >'
        return text
    end
    ...
    def context_links obj
       if obj.container_obj then
          tags = []
          owner_obj = obj.container_obj
          while owner_obj do 
             tags << link_to(owner_obj.iname, url_show(owner_obj), { :title => owner_obj.display_name } )
             owner_obj = owner_obj.container_obj
          end
          rstr = content_tag(:p, "#{obj.container_obj.display_name}: " + tags.reverse.join(" > "), :class => 'context')
       else
          rstr = ""
       end
    ...
    def rc_format(text,attr_name="")
       while m = /\{image:([^}]+)\}/.match(text) do
          fname = m[1].strip
          if image_exist?(fname) then 
            linktext = image_tag(image_tagpath(fname))
          else
       ...
       while m = /![><]?(\{[^}]+\})?(\w[^.]+\.\w\w\w)!/.match(text) do
          ...
          if image_exist?(sfname) then
            img_path = image_path(image_tagpath(sfname))
       ...
       while m = /\{doc:([^}]+)\}/.match(text) do
          ...
          if doc_exist?(fname) then 
            linktext = link_to fname, image_path(fname).gsub("images","user_docs#{pub_segment()}")
          else
    ...
    def image_tag_or_crlink(img_name,alt_text="Image",width=nil)
        ...
        if File.exists?(image_filepath(img_name)) then
            return width ? image_tag(image_tagpath(img_name), :border => :none, :alt => alt_text, :style => "max-width:#{width.to_s};") :
                           image_tag(image_tagpath(img_name), :alt => alt_text)
        else
   ...
   def labeled_level_for(nodes_hash,opts,level)
     ...
     if nodes.size > 0 then
       ...
       rstr = rstr + <<-EOS1
         <div class="tree_lev1" id="topic-#{@ord}">
           #{expand_ctrl(title,nodes)}&nbsp;#{title + (title_link ? (" "+link_to("("+nodes.size.to_s+")", title_link)) : "")}
         </div>
       EOS1

       rstr = rstr + <<-EOS2
         <div style="display:none;" class="tree_lev2" id="t_#{title}-#{@ord}">
       EOS2
   ...
   def tree_for(node, opts={:action => "show"}, level=0)
     ...
     return <<-EOS
       <div class="tree_lev1">
         <span class="tree_expand_ctrl" onclick="Element.toggle('t_#{node.id}-#{@ord}'); 
               this.innerHTML = $('t_#{node.id}-#{@ord}').style.display == 'none' ? '+': '&#x2011;';"
               >+</span>&nbsp;#{link_to(node.iname, {:id => node,:controller => node.ctrlr_name}.merge(opts))}
       </div>
       <div style="display:none;" class="tree_lev2" id="t_#{node.id}-#{@ord}">
   ...
   def edit_icon_for(obj)
     return link_to(image_tag("pencil.gif", :height => "12", :border=>'none'), url_edit(obj), :style=>"background:inherit;", :title => 'Edit')
   end
  ...
  #
  # <%
  #   obj_links = Proc.new { |obj|
  #       [ link_to("Edit", :action => 'edit_user', :id => obj),
  #       " | ",
  #       link_to("Delete", :action => 'delete_user', :id => obj) ]
  #     }
  # %>
  # 
  # <%= table_for(@client_users,
  #   { :table_attrs => {:border => '0', :cellspacing => '1', :style => ''},
  #     :cols => [
  #       {:attr => "last_name", :label => "Last Name"},
  #       {:attr => "first_name", :label => "First Name"},
  #       {:attr => "email", :label => "E-Mail"},
  #       {:proc => obj_links, :label => "&nbsp;"},
  #     ]
  #   })
  # %>
  #
  ...
  def table_for(items, args)
    args = {:header => true}.merge(args)
    
    table_attrs = table_defaults().merge(args[:table_attrs]||{})

    tstr = tag(:table, table_attrs, true)

    if args[:header]
      tstr << content_tag(:tr,
                            args[:cols].collect{|c|
                              content_tag(:th, c[:label]||'&nbsp;', c[:th_attrs]||{})
                            }.join("\n"))
    end
