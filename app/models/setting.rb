...
class Setting < ActiveRecord::Base
   ...
   def self.value_for(name, default="", auto_add=false)
      setting = self.find_by_name(name)
      if setting.nil? && auto_add then
         setting = self.create({:name => name, :value => default})
      end
    ...
    def self.attr_type_info(attr_name=nil)
        unless defined?(@@attr_info)
          @@attr_info = {}
          @@attr_info['name'] = { :datatype => 'string', :modifier => '', :length => 40, :is_identity_attr => true, :default => '', :mode => 'rw' }
 ...
 public
  ...
  def next_item
    item = Setting.find(:first, :order => FtUtils.quoted_name('name'), :conditions => ["name > ? ", self.name] )
    item = Setting.find(:first, :order => FtUtils.quoted_name('name')) if item.nil? 
    return item
  end

  def prev_item
    item = Setting.find(:first, :order => FtUtils.quoted_name('name') + ' DESC', :conditions => ["name < ? ", self.name] )
    item = Setting.find(:first, :order => FtUtils.quoted_name('name') + ' DESC') if item.nil? 
    return item
  end

  def self.items_for_index
    return Setting.find(:all, :order => 'name')
  end
  ...
  def self.default_query(cond=nil)
    ...
    return <<-EOS
          (SELECT setting.name as #{FtUtils.quoted_name('name')},
              setting.value as #{FtUtils.quoted_name('value')},
              setting.id as id
          FROM
              #{FtUtils.quoted_name('settings')} AS #{FtUtils.quoted_name('setting')}
  ...
  def self.query_specs
    ...
    @@query_specs = {
      "list_all" => 
        { :title => "List of "+"Setting".pluralize,
          :sql  => "SELECT * FROM #{@@default_query}",
          :cols => Setting.value_attrs.concat(Setting.foreign_key_attrs),
  ...
  def get_attr_val(ft_col_name,ctx='html')
    attr_info = Setting.attr_type_info(ft_col_name)
