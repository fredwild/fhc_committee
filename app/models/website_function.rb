...
class WebsiteFunction < ActiveRecord::Base

   include FtUtils

   set_table_name 'website_functions'
 ...
 public
  ...
  def next_item
    item = WebsiteFunction.find(:first, :order => FtUtils.quoted_name('name'), :conditions => ["name > ? ", self.name] )
    item = WebsiteFunction.find(:first, :order => FtUtils.quoted_name('name')) if item.nil? 
    return item
  end

  def prev_item
    item = WebsiteFunction.find(:first, :order => FtUtils.quoted_name('name') + ' DESC', :conditions => ["name < ? ", self.name] )
    item = WebsiteFunction.find(:first, :order => FtUtils.quoted_name('name') + ' DESC') if item.nil? 
    return item
  end

  def self.items_for_index
    return WebsiteFunction.find(:all, :order => 'name')
  end
  ...
  def self.default_query(cond=nil)
    ...
    return <<-EOS
          (SELECT website_function.name as #{FtUtils.quoted_name('name')},
              website_function.description as #{FtUtils.quoted_name('description')},
              website_function.link as #{FtUtils.quoted_name('link')},
              website_function.seq_no as #{FtUtils.quoted_name('seq_no')},
              task.name as #{FtUtils.quoted_name('task')},
              task.id as task_task_obj_id,
              website_function.id as id
          FROM
              #{FtUtils.quoted_name('website_functions')} AS #{FtUtils.quoted_name('website_function')}
              left outer join #{FtUtils.quoted_name('task_objs')} as #{FtUtils.quoted_name('task')} on website_function.task_task_obj_id = task.id
          #{cond ? ("WHERE "+cond.to_s) : ""}
          ) AS website_functions
          EOS
  end
  ...
  def self.query_specs
    ...
    @@query_specs = {
      "list_all" => 
        { :title => "List of "+"Website Function".pluralize,
          :sql  => "SELECT * FROM #{@@default_query}",
          :cols => WebsiteFunction.value_attrs.concat(WebsiteFunction.foreign_key_attrs),
  ...
  def get_attr_val(ft_col_name,ctx='html')
    attr_info = WebsiteFunction.attr_type_info(ft_col_name)
  ...
  def self.choices_for(obj,rel_name=nil)
    return WebsiteFunction.find(:all,   :order => 'seq_no,name')
  end
