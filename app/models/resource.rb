...
class Resource < ActiveRecord::Base
   ...
   has_and_belongs_to_many :meeting, :class_name => "Meeting", :join_table => "meetings_resources_resources", :association_foreign_key => "meeting_src_id", :foreign_key => "resource_dest_id"

   has_and_belongs_to_many :tasks, :class_name => "TaskObj", :join_table => "task_objs_resources_resources", :association_foreign_key => "task_obj_src_id", :foreign_key => "resource_dest_id"
   ...
   def get_children
    ...
    def self.attr_type_info(attr_name=nil)
        unless defined?(@@attr_info)
          ...
          @@attr_info['link'] = { :datatype => 'text', :modifier => 'linktext', :is_identity_attr => false, :default => '', :mode => 'rw' }
    ...
    def self.value_attrs
      ["name","description","doc_file","link","seq_no"]
    end
    ...
    def self.get_form_fields()
       ["name","description","picture","doc_file","link","seq_no"]
    end
 ...
 public
  ...
  def next_item
    item = Resource.find(:first, :order => FtUtils.quoted_name('name'), :conditions => ["name > ? ", self.name] )
    item = Resource.find(:first, :order => FtUtils.quoted_name('name')) if item.nil? 
    return item
  end

  def prev_item
    item = Resource.find(:first, :order => FtUtils.quoted_name('name') + ' DESC', :conditions => ["name < ? ", self.name] )
    item = Resource.find(:first, :order => FtUtils.quoted_name('name') + ' DESC') if item.nil? 
    return item
  end

  def self.items_for_index
    return Resource.find(:all, :order => 'name')
  end
  ...
  def self.default_query(cond=nil)
    ...
    return <<-EOS
          (SELECT resource.name as #{FtUtils.quoted_name('name')},
              resource.description as #{FtUtils.quoted_name('description')},
              resource.picture as #{FtUtils.quoted_name('picture')},
              resource.doc_file as #{FtUtils.quoted_name('doc_file')},
              resource.link as #{FtUtils.quoted_name('link')},
              resource.seq_no as #{FtUtils.quoted_name('seq_no')},
              resource.id as id
          FROM
              #{FtUtils.quoted_name('resources')} AS #{FtUtils.quoted_name('resource')}
  ...
  def self.query_specs
    ...
    @@query_specs = {
      "list_all" => 
        { :title => "List of "+"Resource".pluralize,
          :sql  => "SELECT * FROM #{@@default_query}",
          :cols => Resource.value_attrs.concat(Resource.foreign_key_attrs),
  ...
  def get_attr_val(ft_col_name,ctx='html')
    attr_info = Resource.attr_type_info(ft_col_name)
  ...
  def self.choices_for(obj,rel_name=nil)
    return Resource.find(:all,   :order => 'seq_no,name')
  end
